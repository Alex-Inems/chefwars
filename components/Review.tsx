...
const Review: React.FC = () => {
  ...
  return (
    <div className="bg-orange-500 p-6 rounded-lg shadow-lg w-full  flex">
      {/* Review Text Section with Slide Effect */}
      <div className="w-1/2 p-4 text-center flex flex-col justify-center">
        <p className="text-gray-800 text-lg transition-opacity duration-500">
          {reviews[currentReview].text}
        </p>
        <hr className="border-dotted border-gray-400 my-4" />
        <div className="flex items-center mt-4">
          <img src={reviews[currentReview].image} alt={reviews[currentReview].name} className="w-10 h-10 rounded-full mr-3" />
          <div className='text-center justify-center'>
            <p className="text-gray-900 font-semibold">{reviews[currentReview].name}</p>
            <p className="text-gray-600 text-sm">{reviews[currentReview].title}</p>
          </div>
        </div>
      </div>
